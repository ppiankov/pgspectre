...
PostgreSQL permission and exposure scanner.
...
```
go install github.com/ppiankov/pgspectre/cmd/pgspectre@latest
```
...
Scans PostgreSQL databases for security findings.
...
- Does not remediate or modify PostgreSQL databases — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated PostgreSQL databases monitoring — point-in-time security audit only
...
```bash
pgspectre scan --output json | jq '.summary'
pgspectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
